...
async def register_user(user: auth.UserCreate, background_tasks: BackgroundTasks):
    existing_user = await usercollection.find_one({"email": user.username})
    if existing_user:
...
@app.post('/trip/place/photos')
async def photos(param: photoModel, current_user: auth.UserInDB = Depends(current_active_user_dependency)):
